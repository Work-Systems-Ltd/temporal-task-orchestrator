"""Workflow graph building — parent/child tree with activity and task nodes."""
...
from ui.services.temporal.tasks import get_pending_task


async def get_workflow_graph(
    client: Client, workflow_id: str, detail: WorkflowDetail,
) -> GraphNode | None:
    ...
    """
    from ui.services.temporal.detail import get_workflow_detail

    root_id = detail.parent_id or workflow_id
    root_detail = detail if not detail.parent_id else await get_workflow_detail(client, root_id)
    if not root_detail:
        return None

    root = await _build_graph_node(client, root_id, root_detail.workflow_type, root_detail.status, workflow_id)
    return root if root.children else None


async def get_all_pending_tasks(
    client: Client, graph: GraphNode | None, workflow_id: str,
) -> list[dict]:
    ...
    def _collect_running(node: GraphNode) -> None:
        if node.node_type == "task":
            return  # task nodes share parent's workflow_id
    ...
    async def _fetch(wid: str) -> dict | None:
        meta = await get_pending_task(client, wid)
        if meta:
...
async def _find_children(client: Client, parent_wf_id: str) -> list[tuple[str, str, str]]:
    """Return (wf_type, child_wf_id, status) for direct children of a workflow."""
    try:
        handle = client.get_workflow_handle(parent_wf_id)
...
async def _build_graph_node(
    ...
    if depth < max_depth:
        child_infos = await _find_children(client, wf_id)
        if child_infos:
            node.children = await asyncio.gather(*[
                _build_graph_node(client, cid, ctype, cstatus, current_id, depth + 1, max_depth)
    ...
    try:
        ...
        if node.status == "running" and not has_human_task:
            try:
                meta = await get_pending_task(client, wf_id)
                if meta:
                    node.children.append(GraphNode(
                        workflow_id=wf_id, workflow_type=wf_type,
