"""Workflow and pending task listing, counting, and deduplication."""
...
from ui.services.temporal.helpers import build_query, is_assigned_to_user
from ui.services.temporal.tasks import sanitize_assignment


async def count_workflows(client: Client, query: str | None) -> int:
    """Count workflows matching *query*, deduplicating by workflow ID."""
    seen: dict[str, str] = {}
    async for wf in client.list_workflows(query, page_size=100):
...
async def count_pending(client: Client, wf_type: str | None = None) -> int:
...
async def get_tab_counts(
    ...
    async def _count_tab(tab: str) -> tuple[str, int]:
        if tab == "pending":
            return tab, await count_pending(client, wf_type)
        q = build_query(STATUS_QUERIES[tab], wf_type)
        return tab, await count_workflows(client, q)
...
async def list_pending(
    ...
    async for wf in client.list_workflows(query):
        try:
            ...
            meta = await sanitize_assignment(TaskMeta.model_validate_json(raw))
...
async def deduplicate_runs(client: Client, items: list[WorkflowItem]) -> list[WorkflowItem]:
...
async def list_workflows(
    ...
    from ui.services.temporal.helpers import group_by_parent
    ...
    async for wf in client.list_workflows(query, page_size=size * 4):
        if search:
    ...
    deduped = await deduplicate_runs(client, items[:size])
