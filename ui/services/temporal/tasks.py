"""Pending task queries, signals, and assignment sanitization."""
...
async def get_pending_task(client: Client, workflow_id: str) -> TaskMeta | None:
    ...
    return await sanitize_assignment(meta)


async def signal_complete(client: Client, workflow_id: str, data: str) -> None:
    """Signal a workflow that its human task has been completed."""
    handle = client.get_workflow_handle(workflow_id)
    await handle.signal(WorkSysFlow.complete_human_task, data)


async def signal_reassign(
    client: Client,
    workflow_id: str,
    assigned_user: str = "",
    assigned_group: str = "",
...
async def sanitize_assignment(meta: TaskMeta) -> TaskMeta:
